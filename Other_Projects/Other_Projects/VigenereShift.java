/**
 * The VigenereShift class extends the Vigenere class and implements the 
 * Hasher interface.
 * It's purpose is to hash a password.
 * @author MIDN 3/C NICHOLAS ZAYFMAN
 */
public class VigenereShift extends Vigenere implements Hasher{
    
    //Private field to store the password.
    private static char[] psswd;
    
    /**
     * This method returns the algorithm name: shift+vigenere.
     * @return "shigt+vigenere"
     */
    ...
    /**
     * This method shift+ hashes the password using the Vigenere method.
    ...
    public String hash(String initVect) throws IndexOutOfBoundsException{
        ...
        for(int j = 0; j < maxcount; j++){
             ...
             * the Vigenere Encryption function.
             */
            if(j!=maxcount-1){
                super.init(((new String(psswd)).substring(j*16, j*16 + 16)).toCharArray());
            }
            else{
                if(psswd.length%16==0){
                    super.init(((new String(psswd)).substring(j*16, j*16 + 16)).toCharArray());
                ...
                super.init(((new String(psswd)).substring(j*16, j*16 + psswd.length%16)).toCharArray());
                }
            }
    ...
    /**
     * This method sets the key in the VigenereShift class to the inputed key.
